...
@app.route('/gesture')
def gesture():
    return Response(gesture_(),mimetype='multipart/x-mixed-replace; boundary=frame')

@app.route('/')
def index():
    # while registrar.get("printText") == True:
    #     registrar.add("printText", False)
    #     return render_template('index.html', printText='True')
    #hello = get_template_attribute('index.html', 'hello')
    #return get_template_attribute('index.html', 'hello')
    #printer_ = get_template_attribute('index.html', 'printer')
    # registrar.add("printer", printer_)
...
一つのサーバーで2タブ開いて、片方のタブからもらった情報（テキスト）をもう片方に流す